...
Framer Motion requires browser APIs, so animated sections become `"use client"`. All four section files are leaf components with no server-only data fetching, so this is safe.
